...
const SalesVisual = () => {
  const orders = [
    { id: "#ORD-4821", customer: "Apex Retail Co.", amount: "$3,240", status: "Completed", color: "text-green-600 bg-green-50" },
    { id: "#ORD-4820", customer: "BlueLine Store", amount: "$1,850", status: "Processing", color: "text-yellow-600 bg-yellow-50" },
    { id: "#ORD-4819", customer: "Nova Traders", amount: "$5,120", status: "Completed", color: "text-green-600 bg-green-50" },
  ];
    ...
    <div className="bg-[#F8FAFC] rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 relative flex flex-col gap-4 min-h-[300px] md:min-h-[400px] overflow-hidden justify-center">
      ...
      <div className="relative z-10 bg-white rounded-2xl border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-2xl font-bold tracking-tight text-gray-900">$24,380</p>
        </div>
          ...
          {orders.map((order) => (
            <div key={order.id} className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-700 font-medium hidden sm:inline">{order.customer}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="font-semibold text-gray-900">{order.amount}</span>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${order.color}`}>{order.status}</span>
              </div>
            </div>
          ))}
        </div>
...
const PurchaseVisual = () => {
  const items = [
    { name: "Office Chairs (x20)", supplier: "FurniPro Ltd.", value: "$2,400", icon: "📦" },
    { name: "Laptop Units (x10)", supplier: "TechSource", value: "$12,000", icon: "💻" },
    { name: "Stationery Pack", supplier: "OfficeWorld", value: "$340", icon: "✏️" },
  ];
    ...
    <div className="bg-[#F8FAFC] rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 relative flex flex-col gap-4 min-h-[300px] md:min-h-[400px] overflow-hidden justify-center">
      ...
      <div className="relative z-10 bg-white rounded-2xl border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 md:p-5">
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div key={item.name} className="flex items-center justify-between text-xs py-1.5 border-t border-gray-50">
              <div className="flex items-center gap-2">
                <span>{item.icon}</span>
                <div>
...
const StockVisual = () => {
  const stockItems = [
    { name: "Wireless Keyboards", qty: 142, max: 200, color: "bg-green-400", status: "Healthy" },
    { name: "USB-C Hubs", qty: 23, max: 100, color: "bg-yellow-400", status: "Low Stock" },
    { name: "Monitor Stands", qty: 8, max: 80, color: "bg-red-400", status: "Critical" },
    { name: "Desk Lamps", qty: 67, max: 100, color: "bg-blue-400", status: "Good" },
  ];
  ...
  return (
    <div className="bg-[#F8FAFC] rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 relative flex flex-col gap-4 min-h-[300px] md:min-h-[400px] overflow-hidden justify-center">
      ...
      <div className="relative z-10 flex gap-3">
        {[
          { label: "Total SKUs", value: "1,284", icon: <Boxes size={13} className="text-teal-500" /> },
          { label: "Low Stock", value: "14 items", icon: <BarChart3 size={13} className="text-orange-500" /> },
        ].map((s) => (
          <div key={s.label} className="flex-1 bg-white rounded-2xl border border-gray-100/80 shadow-[0_4px_15px_rgb(0,0,0,0.03)] p-3.5 flex items-center gap-2">
            {s.icon}
            <div>
      ...
      {/* Stock levels */}
      <div className="relative z-10 bg-white rounded-2xl border border-gray-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 md:p-5">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Stock Levels</p>
            ...
            <div key={item.name}>
              <div className="flex items-center justify-between text-xs mb-1">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-gray-400">{item.qty}/{item.max}</span>
                  <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                    item.status === "Critical" ? "bg-red-50 text-red-500" :
                    item.status === "Low Stock" ? "bg-yellow-50 text-yellow-600" :
